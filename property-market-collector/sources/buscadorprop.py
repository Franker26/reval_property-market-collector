...
_BASE = "https://www.buscadorprop.com.ar"
...
class BuscadorpropSource(BaseSource):
    @staticmethod
    def can_handle(url: str) -> bool:
        return "buscadorprop.com.ar" in url

    async def extract(self, url: str, client: httpx.AsyncClient) -> dict:
        html = await fetch_html(url, client, "BuscadorProp")
        result = parse_ldjson(html)
        if not result:
            result = parse_body_text(html)
        if not result:
            raise HTTPException(422, "No se pudieron extraer datos de BuscadorProp")
        return result
    ...
    async def search_listings(
        ...
        for page in range(1, paginas + 1):
            ...
            if page > 1:
                params["pagina"] = page
            if precio_min:
                params["precio_desde"] = precio_min
            if precio_max:
                params["precio_hasta"] = precio_max
            if ambientes_min:
            ...
            try:
                html = await fetch_html(search_url, client, "BuscadorProp", params=params)
            except HTTPException:
                continue

            # Listing URLs: /propiedad/{id}-{slug}/
            page_urls = extract_href_urls(html, _BASE, r"^/propiedad/\d+")
            if not page_urls:
                break
